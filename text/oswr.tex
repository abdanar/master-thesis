...
\end{align*}
This result holds for a general lifting function, and depending on the choice, it may reduce to a simpler form. In particular, if we use harmonic lifting, the estimate becomes
\begin{equation*}
...
\vspace{10pt}
\noindent Before we proceed, it is better to include certain intermediate observations. Since $P_{j, r}$ is a best approximation in the $V_{j}$-norm, we have
\begin{equation*}
    \dfrac{1}{\sqrt{1 + C_{P}^{2}}}\left\lVert\eta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} \leq \left\lVert\eta_{j}^{k+1}(\cdot, t)\right\rVert_{V_{j}} = \lVert w_j^{k+1} - P_{j,r} w_j^{k+1} \rVert_{V_j} = \inf_{v_r \in V_{j,r}} \lVert w_j^{k+1} - v_r \rVert_{V_j}.
\end{equation*}
...
\begin{align*}
    ...
    &\quad +\left\lVert\delta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}e^{T/2}\left(\int_{0}^{t}2\left\lVert \partial_{t}\delta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})} +\lVert \nabla \delta_{j}^{k+1}\rVert^{2}_{L^{2}(\Omega_{j})}\,ds\right)^{1/2},
\end{align*}
or
\begin{align*}
    \left\lVert e_{j, r}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} &\leq \left\lVert\eta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\eta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})}ds\right)^{1/2}\\
...
\end{align*}
For harmonic lifting, this estimate reduces to
\begin{align*}
    \left\lVert e_{j, r}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} &\leq \left\lVert\eta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\eta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})}ds\right)^{1/2}\\
    &\quad +\left\lVert\delta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\delta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})} ds\right)^{1/2}.
\end{align*}
This estimate clearly shows the effect of the proposed reduced-order compatible lifting approach. We are not done! An important part is to replace these additional terms with the previous iteration errors, possibly from the neighbouring subdomains. Here, we will consider two cases discussed above: the Additive Schwarz and the Restricted Additive Schwarz methods: Lifting functions $l_{j}^{k+1}(\cdot, t), l_{j, r}^{k+1}(\cdot, t)\in H^{1}(\Omega_{j})$ is defined by
\begin{equation*}
...
where
\begin{itemize}
    \item[--] for the Additive Schwarz method, the full and reduced boundary functions are defined by
    \begin{equation*}
        \psi_{j}^{k}(x, t) := 
        \begin{cases}
            \sum_{l\in\mathcal{I}}u_{l}^{k}(x, t), & x\in \Gamma_{j}^{\mathcal{I}},\\
        ...
        \psi_{j,r}^{k}(x, t) := 
        \begin{cases}
            \sum_{l\in\mathcal{I}}u_{l,r}^{k}(x, t), & x\in \Gamma_{j}^{\mathcal{I}},\\
            g(x, t), & x\in \Gamma_{j0},
        \end{cases}
    \end{equation*}
    \item[--] for the Restricted Additive Schwarz method, the full and reduced boundary functions are defined by
    \begin{equation*}
        \psi_{j}^{k}(x, t) := 
        \begin{cases}
            \sum_{l\in\mathcal{I}}w_{l}u_{l}^{k}(x, t), & x\in \Gamma_{j}^{\mathcal{I}},\\
        ...
        \psi_{j,r}^{k}(x, t) := 
        \begin{cases}
            \sum_{l\in\mathcal{I}}w_{l}u_{l,r}^{k}(x, t), & x\in \Gamma_{j}^{\mathcal{I}},\\
            g(x, t), & x\in \Gamma_{j0},
        \end{cases}
    \end{equation*}
    on a disjoint decomposition of the boundary up to measure zero
    \begin{equation*}
        \partial\Omega_{j} = \bigcup_{\emptyset \neq \mathcal{I}\subset\mathcal{N}_{j}}\Gamma_{j}^{\mathcal{I}}, \qquad \Gamma_{j}^{\mathcal{I}} = \bigcap_{l\in\mathcal{I}}\Gamma_{jl}\setminus\bigcup_{k\in\mathcal{N}_{j}\setminus\mathcal{I}}\Gamma_{jk}.
    \end{equation*}
\end{itemize}
...
\end{equation*}
Next step is to compute $\lVert(\psi_{j}^{k} - \psi_{j, r}^{k})(\cdot, t)\rVert_{H^{1/2}(\partial\Omega_{j})}$. Considering the boundary functions defined above,
\begin{itemize}
    \item[--] for the Additive Schwarz method, we get
    \begin{equation*}
        \psi_{j}^{k}(x, t) - \psi_{j,r}^{k}(x, t) = 
        \begin{cases}
            \sum_{l\in\mathcal{I}}\left(u_{l}^{k}(x, t) - u_{l, r}^{k}(x, t)\right), & x\in \Gamma_{j}^{\mathcal{I}},\\
            0, & x\in \Gamma_{j0},
        \end{cases},
    \end{equation*}
    \item[--] for the Restricted Additive Schwarz method, we get
    \begin{equation*}
        \psi_{j}^{k}(x, t) - \psi_{j,r}^{k}(x, t) = 
        \begin{cases}
            \sum_{l\in\mathcal{I}}w_{l}\left(u_{l}^{k}(x, t) - u_{l, r}^{k}(x, t)\right), & x\in \Gamma_{j}^{\mathcal{I}},\\
            0, & x\in \Gamma_{j0}.
        \end{cases}
    \end{equation*}
\end{itemize}
Once we use the definition of $H^{1/2}$ norm on the boundary $\partial\Omega_{j}$, we obtain
\begin{equation*}
    \lVert(\psi_{j}^{k} - \psi_{j, r}^{k})(\cdot, t)\rVert^{2}_{H^{1/2}(\partial\Omega_{j})} \leq \sum_{\mathcal{I}\subset\mathcal{N}_{j}}\sum_{l\in\mathcal{I}}\left\lVert \left(u_{l}^{k} - u_{l, r}^{k}\right)(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})}^{2} \leq C_{m}\sum_{l\in\mathcal{N}_{j}}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})}^{2},
\end{equation*}
or using $\sqrt{a+b}\leq\sqrt{a}+\sqrt{b}$,
\begin{equation*}
    \lVert(\psi_{j}^{k} - \psi_{j, r}^{k})(\cdot, t)\rVert_{H^{1/2}(\partial\Omega_{j})} \leq \sqrt{C_{m}}\sum_{l\in\mathcal{N}_{j}}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})},\footnote{It’s less sharp than the root-of-sum-of-squares version, and it overestimates slightly.}
\end{equation*}
which implies
\begin{equation*}    
\lVert\delta_{j}^{k+1}(\cdot, t)\rVert_{H^{1}(\Omega_{j})} \leq C_{E}\sqrt{C_{m}}\sum_{l\in\mathcal{N}_{j}}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})}.
\end{equation*}
Similarly, we obtain
\begin{equation*}
    \lVert\partial_{t}(\psi_{j}^{k} - \psi_{j, r}^{k})(\cdot, t)\rVert^{2}_{H^{1/2}(\partial\Omega_{j})}\leq C_{m}\sum_{l\in\mathcal{N}_{j}}\left\lVert \partial_{t}e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})}^{2},
\end{equation*}
which implies
\begin{equation*}
    \lVert\partial_{t}\delta_{j}^{k+1}(\cdot, t)\rVert^{2}_{L^{2}(\Omega_{j})} \leq \lVert\partial_{t}\delta_{j}^{k+1}(\cdot, t)\rVert^{2}_{H^{1}(\Omega_{j})} \leq C_{E}^{2}C_{m}\sum_{l\in\mathcal{N}_{j}}\left\lVert \partial_{t}e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})}^{2}.
\end{equation*}
Also, we have
\begin{equation*}
    \lVert\nabla\delta_{j}^{k+1}(\cdot, t)\rVert^{2}_{L^{2}(\Omega_{j})} \leq \lVert\delta_{j}^{k+1}(\cdot, t)\rVert^{2}_{H^{1}(\Omega_{j})} \leq C_{E}^{2}C_{m}\sum_{l\in\mathcal{N}_{j}}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})}^{2}.
\end{equation*}
Once we use all the inequalities, we obtain
\begin{align*}
    \left\lVert\delta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} &+ C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\delta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})} +\lVert \nabla \delta_{j}^{k+1}\rVert^{2}_{L^{2}(\Omega_{j})}\,ds\right)^{1/2} \leq C_{E}\sqrt{C_{m}}\sum_{l\in\mathcal{N}_{j}}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})} +\\
    &\quad + C_{eq}\sqrt{2}C_{E}\sqrt{C_{m}}e^{T/2}\left(\sum_{l\in\mathcal{N}_{j}}\int_{0}^{t}\left\lVert \partial_{t}e_{l, r}^{k}(\cdot, s)\right\rVert_{H^{1/2}(\Gamma_{jl})}^{2} + \left\lVert e_{l, r}^{k}(\cdot, s)\right\rVert_{H^{1/2}(\Gamma_{jl})}^{2}\,ds\right)^{1/2}.
\end{align*}
Thus, the general estimate reduces to
\begin{align*}
    \left\lVert e_{j, r}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} &\leq \left\lVert\eta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\eta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})}ds\right)^{1/2}+ C_{j}\sum_{l\in\mathcal{N}_{j}}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})}+\\
    &\quad + C_{eq}\sqrt{2}C_{j}e^{T/2}\left(\sum_{l\in\mathcal{N}_{j}}\int_{0}^{t}\left\lVert \partial_{t}e_{l,r}^{k}(\cdot, s)\right\rVert^{2}_{H^{1/2}(\Gamma_{jl})} +\left\lVert e_{l,r}^{k}(\cdot, s)\right\rVert^{2}_{H^{1/2}(\Gamma_{jl})}\,ds\right)^{1/2},
\end{align*}
where $C_{j} = C_{E}\sqrt{C_{m}}$. Before making an important remark, let us discuss the initialization of the iterative method, i.e., $u_{j}^{0}$ and $u_{j, r}^{0}$. By definition, we have
\begin{equation*}
    u_{j}^{0}(x, t) = w_{j}^{0}(x, t) + l_{j}^{0}(x, t), \qquad u_{j, r}^{0}(x, t) = w_{j, r}^{0}(x, t) + l_{j, r}^{0}(x, t),
\end{equation*}
where $w_{j}^{0}\in V_{j} = H_{0}^{1}(\Omega_{j})$, $w_{j, r}^{0}\in V_{j, r}\subset H_{0}^{1}(\Omega_{j})$, $l_{j}^{0}\in H^{1}(\Omega_{j})$, and $l_{j, r}^{0}\in H^{1}(\Omega_{j})$. The common choice for OWR is
\begin{equation*}
    w_{j}^{0}(x, t) = 0, \qquad l_{j}^{0}(x, t) = E_{\text{Tr}}(\psi_{j}^{0}(\cdot, t)),
\end{equation*}
where
\begin{equation*}
    \psi_{j}^{0}(x, t) = 
    \begin{cases}
        0 & x\in \Gamma_{jl},\, l\in \mathcal{N}_{j},\\
        g(x, t) & x\in \Gamma_{j0}.
    \end{cases}
\end{equation*}
However, one is free to choose $w_{j}^{0}(x, t)$, and it is important to note that this choice affects convergence. For ROWR, assuming the common choice above, we set
\begin{equation*}
    w_{j, r}^{0}(x, t) = P_{j, r}w_{j}^{0}(x, t) = 0, \qquad l_{j, r}^{0}(x, t) = l_{j}^{0}(x, t).
\end{equation*}
Considering the assumptions above, it follows that
\begin{equation*}
    e_{l,r}^{0}(x, t) = u_{l}^{0}(x, t) - u_{l, r}^{0}(x, t) = 0.
\end{equation*}
Then the estimate for the first iteration reads
\begin{equation*}
    \left\lVert e_{j, r}^{1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} \leq \left\lVert\eta_{j}^{1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\eta_{j}^{1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})}ds\right)^{1/2}, \quad \text{for all } j = 1, \ldots, N.
\end{equation*}
This shows that the quality of the surrogate model in the first iteration directly influences the overall convergence of the iteration: the more accurate the initial reduced-order model, the faster and more accurately the subsequent iterations progress.

\vspace{10pt}
\noindent Let us recall that for $v(\cdot, t)\in H^{1}(\Omega_{l})$, we have the trace inequality
\begin{equation*}
    \left\lVert \text{Tr}_{\Gamma_{jl}}v(\cdot, t)\right\rVert_{H^{1/2}(\Gamma_{jl})} \leq C_{tr, jl}\left\lVert v(\cdot, t)\right\rVert_{H^{1}(\Omega_{l})}.
\end{equation*}
Then, the general estimate becomes
\begin{align*}
    \left\lVert e_{j, r}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} &\leq \left\lVert\eta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\eta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})}ds\right)^{1/2}+ C_{j}\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1}(\Omega_{l})}+\\
    &\quad + C_{eq}\sqrt{2}C_{j}e^{T/2}\left(\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}^{2}\int_{0}^{t}\left\lVert \partial_{t}e_{l,r}^{k}(\cdot, s)\right\rVert^{2}_{H^{1}(\Omega_{l})} + \left\lVert e_{l,r}^{k}(\cdot, s)\right\rVert^{2}_{H^{1}(\Omega_{l})}\,ds\right)^{1/2}.
\end{align*}
Let us introduce
\begin{align*}
    E_{j, r}^{k+1}(t) &= \left\lVert e_{j, r}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})},\\
    R_{j}^{k+1}(t) &= \left\lVert\eta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\eta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})}ds\right)^{1/2},\\
    L_{j, r}^{k}(t) &= C_{j}\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1}(\Omega_{l})} + C_{eq}\sqrt{2}C_{j}e^{T/2}\left(\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}^{2}\int_{0}^{t}\left\lVert \partial_{t}e_{l,r}^{k}(\cdot, s)\right\rVert^{2}_{H^{1}(\Omega_{l})} + \left\lVert e_{l,r}^{k}(\cdot, s)\right\rVert^{2}_{H^{1}(\Omega_{l})}\,ds\right)^{1/2}.
\end{align*}
Note that if we take a \textbf{harmonic lifting}, then $L_{j}^{k}(t)$ reduces to
\begin{equation*}
    L_{j, r}^{k}(t) = C_{j}\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1}(\Omega_{l})} + C_{eq}\sqrt{2}C_{j}e^{T/2}\left(\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}^{2}\int_{0}^{t}\left\lVert \partial_{t}e_{l,r}^{k}(\cdot, s)\right\rVert^{2}_{H^{1}(\Omega_{l})}\,ds\right)^{1/2},
\end{equation*}
whereas for a \textbf{parabolic lifting}, it reduces to
\begin{equation*}    L_{j, r}^{k}(t) = C_{j}\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1}(\Omega_{l})}.
\end{equation*}
Notice that in the case of parabolic lifting, we have 
\begin{equation*}
    L_{j, r}^{k}(t) = C_{j}\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}E_{l, r}^{k}(t).
\end{equation*}
For the sake of simplicity, let us consider the parabolic lifting case, i.e.,
\begin{align*}
    \left\lVert e_{j, r}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} \leq \left\lVert\eta_{j}^{k+1}(\cdot, t)\right\rVert_{H^{1}(\Omega_{j})} + C_{eq}\sqrt{2}e^{T/2}\left(\int_{0}^{t}\left\lVert \partial_{t}\eta_{j}^{k+1}(\cdot, s)\right\rVert^{2}_{L^{2}(\Omega_{j})}ds\right)^{1/2}+ C_{j}\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}\left\lVert e_{l, r}^{k}(\cdot, t)\right\rVert_{H^{1}(\Omega_{l})},
\end{align*}
or equivalently,
\begin{equation*}
    E_{j, r}^{k+1}(t) \leq R_{j}^{k+1}(t) + C_{j}\sum_{l\in\mathcal{N}_{j}}C_{tr, jl}E_{l, r}^{k}(t),
\end{equation*}
where for the first iteration we have
\begin{equation*}
    E_{j, r}^{1}(t) \leq R_{j}^{1}(t).
\end{equation*}
For $k=1$,
\begin{equation*}
    E_{j,r}^{2}(t) \leq R_{j}^{2}(t) + C_j \sum_{l \in \mathcal{N}_j} C_{tr,jl}E_{l,r}^{1}(t).
\end{equation*}
Using $E_{l,r}^{1}(t) \leq R_{l}^{1}(t)$, we obtain
\begin{equation*}
    E_{j,r}^{2}(t) \leq R_{j}^{2}(t) + C_j \sum_{l \in\mathcal{N}_j}
    C_{tr,jl}R_{l}^{1}(t).
\end{equation*}
For $k=2$,
\begin{equation*}
    E_{j,r}^{3}(t) \leq R_{j}^{3}(t) + C_j \sum_{l \in\mathcal{N}_j}
    C_{tr,jl} E_{l,r}^{2}(t).
\end{equation*}
Inserting the previous bound yields
\begin{align*}
    E_{j,r}^{3}(t) \leq R_{j}^{3}(t) + C_j \sum_{l \in \mathcal{N}_j}
    C_{tr,jl} \left(R_{l}^{2}(t) + C_l \sum_{m \in \mathcal{N}_l}C_{tr,lm}\, R_{m}^{1}(t)\right).
\end{align*}
Define
\begin{equation*}
    E^{k}(t) = \left(E_{1,r}^{k}(t), \dots, E_{N,r}^{k}(t)\right)^{T}, \qquad R^{k}(t) = \left(R_{1}^{k}(t), \dots, R_{N}^{k}(t)\right)^{T}.
\end{equation*}
Introduce the nonnegative matrix $A=(A_{jl})$ defined by
\begin{equation*}
    A_{jl} =
    \begin{cases}
        C_j C_{tr,jl}, & l \in \mathcal{N}_j, \\
        0, & \text{otherwise}.
    \end{cases}
\end{equation*}
Then the recursion can be written as
\begin{equation*}
    E^{k+1}(t) \leq R^{k+1}(t) + AE^{k}(t),
\end{equation*}
where the inequality is understood componentwise. By induction one obtains
\begin{equation*}
    E^{k+1}(t) \leq \sum_{m=0}^{k} A^{m}R^{k+1-m}(t).
\end{equation*}
If
\begin{equation*}
    R^{k}(t) \leq R(t) \quad \text{for all } k,
\end{equation*}
then
\begin{equation*}
    E^{k+1}(t) \leq \left(I + A + A^2 + \dots + A^k \right)R(t).
\end{equation*}
