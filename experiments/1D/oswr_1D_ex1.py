...
global_fig, global_ax = plot_wireframe(vert1D, time_grid, heat_solution1D.T, xlabel=r'$x$', ylabel=r'$t$', xlim=(0, 1), ylim=(t0, T), zlim=(-1, 1))

# Define the Overlapping Schwarz Waveform Relaxation (OSWR) problem with 2 subdomains and an overlap of 1 layer
...
oswr_solution1D = oswr_problem1D.solve(time_grid = time_grid, lift = 'nodal', theta = 1, maxiter = 25, tol = 1e-9, store_solution = iterations)
...
err_data = np.zeros(len(iterations))
for j in iterations:
...
iterates_fig, _ = plot(iterations, err_data, xlabel = 'Iteration', ylabel = 'Error', 
    xticks = iterations, plot_kwargs = {'linewidth': 0.7, 'linestyle':'-', 'color': 'blue'})

# Save the figures
plt.tight_layout()
global_fig.savefig(f"figures/1D/fem/ex1/fem_{T}.png", dpi = 300, bbox_inches = 'tight')
subdomain_fig.savefig(f"figures/1D/oswr/ex1/subdomains_{T}.png", dpi = 300, bbox_inches = 'tight')
iterates_fig.savefig(f"figures/1D/oswr/ex1/iterates_{T}.png", dpi = 300, bbox_inches = 'tight')
plt.show()
