...
def proj_matrices(r: int, ntime: int, femspace: FEMSpace, roswrproblem: ROSWRProblem, lift, theta: float, solver) -> dict[int, np.ndarray]:
    ...
    snapshot_matrix[femspace.interior_nodes,:] = roswrproblem.heat_problem.solve(time_grid = np.linspace(roswrproblem.heat_problem.t0, roswrproblem.heat_problem.T, ntime), 
                                                lift = lift, theta = theta, solver = solver, g_new = np.zeros((2, ntime)), homogeneous = True)
...
boundary_condition = np.zeros((2, ntime))

# Define the initial condition
initial_condition = np.zeros(femspace1D.mesh.nnodes())
...
global_fig, global_ax = plot_wireframe(vert1D, time_grid, heat_solution1D.T, xlabel=r'$x$', ylabel=r'$t$', title = r'Solution of the 1D Heat Equation', xlim=(0, 1), ylim=(t0, T), zlim=(-1, 1))
global_ax.zaxis.set_rotate_label(False)
global_ax.set_zlabel(r'$u(x,t)$', rotation = 90)

# Define the Reduced Overlapping Schwarz Waveform Relaxation (ROSWR) problem with 2 subdomains and an overlap of 1 layer
roswr_problem1D = ROSWRProblem(heat_problem = problem1D, n = 2, overlap = 1)
...
roswr_solution1D = roswr_problem1D.solve(projs = projs1D, time_grid = time_grid, lift = 'nodal', theta = 1, maxiter = 25, tol = 1e-9, store_solution = iterations)
...
for i, iter in enumerate([1, 2, 3, 4]):
    ...
    plot_wireframe(vert1D, time_grid, Z1.T, ax = axes[i, 0], xlabel=r'$x$', ylabel=r'$t$', xlim=(0, 1), ylim=(t0, T), zlim=(-1, 1))
    plot_wireframe(vert1D, time_grid, Z2.T, ax = axes[i, 0], xlabel=r'$x$', ylabel=r'$t$', xlim=(0, 1), ylim=(t0, T), zlim=(-1, 1))
    plot_wireframe(vert1D, time_grid, Z3.T, ax = axes[i, 1], xlabel=r'$x$', ylabel=r'$t$', xlim=(0, 1), ylim=(t0, T), zlim=(-1, 1))
    plot_wireframe(vert1D, time_grid, Z4.T, ax = axes[i, 1], xlabel=r'$x$', ylabel=r'$t$', xlim=(0, 1), ylim=(t0, T), zlim=(-1, 1))

subdomain_fig.suptitle(r'ROSWR Subdomain Solutions(left) and Errors(right) for Iterations $n = 1, 2, 3, 4$')

# Compute absolute error between the ROSWR solution and the heat solution vs iteration number for different r values
err_data = {}
miniter = 0
r_iterations = list(range(1, 41))
r_values = [2, 5, 7, 10]
for r in r_values:
    projs = proj_matrices(r = r, ntime = 51, femspace = femspace1D, roswrproblem = roswr_problem1D, lift = 'nodal', theta = 1, solver = DirectSolver())
    roswr_solution1D = roswr_problem1D.solve(projs = projs, time_grid = time_grid, lift = 'nodal', theta = 1, maxiter = 40, tol = 1e-12, store_solution = r_iterations)
...
oswr_problem1D = OSWRProblem(heat_problem = problem1D, n = 2, overlap = 1)
oswr_solution1D = oswr_problem1D.solve(time_grid = time_grid, lift = 'nodal', theta = 1, maxiter = 40, tol = 1e-12, store_solution = r_iterations)
...
plot(r_iterations[:miniter], oswr_data[:miniter], ax = ax, plot_kwargs = {'linewidth': 0.7, 'linestyle':'--', 'color': 'red'})
ax.set_xlabel('Iteration')
ax.set_ylabel('Error')
ax.set_xticks(r_iterations[:miniter])
ax.legend([rf'$r = {r}$' for r in r_values] + ['OSWR'])

# Save the figures
plt.tight_layout()
subdomain_fig.savefig(f"figures/1D/roswr/ex1/subdomains_{T}.png", dpi = 300, bbox_inches = 'tight')
iterates_fig.savefig(f"figures/1D/roswr/ex1/iterates_{T}.png", dpi = 300, bbox_inches = 'tight')
plt.show()
