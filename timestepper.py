...
class Theta:
    ...
    def __init__(self, M: MatrixType, A: MatrixType, assembler: Assembler, f: Callable, t0: float, dt: float, dirichlet_bc: dict, theta: float):
    ...
    def step(self, u_n: np.ndarray, F_n: np.ndarray, t_n: float, A_n: MatrixValue | None = None, M_n: MatrixValue | None = None):
        ...
        # f(x, y, t_n+1)
        if self.dim == 2:
            func = lambda x, y: self.f(x, y, t)
        else:  # 1D
            func = lambda x: self.f(x, t)
        ...
        u = np.linalg.solve(lhs, rhs) # u_n+1
