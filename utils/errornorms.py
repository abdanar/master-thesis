...
class ErrorNorms:
    ...
    def _diff(self, elem_index, x_phys, t_index=0):
        ...
        if self.mode == 'fem':
            ...
            return u1_x - self.u_exact(t, *x_phys) if self.femspace.dim == 2 else u1_x - self.u_exact(t, x_phys)
        else:
            return u1_x
        
    def _grad_diff(self, elem_index, x_phys, t_index=0):
    ...
    def l2_error_time(self):
        """
        L2 norm over space and time: sqrt(∫₀ᴛ ∫_Ω |u1 - u2|² dx dt)
        Approximated using the trapezoidal rule in time.
        """
        ...
        for i in range(self.nt - 1):
            ...
            err1 = self.l2_error_at_time(i)
            err2 = self.l2_error_at_time(i+1)
            # Trapezoidal rule in time
            err_time += 0.5*dt*(err1**2 + err2**2)
        return float(np.sqrt(err_time))

    def l2_error_at_time(self, t_index):
        """Compute L2 error at a specific time index."""
        err = 0.0
        for elem_index in range(self.mesh.nelements()):
            phys_elem = self.femspace.get_physical_element(elem_index)
            detJ = phys_elem.det_jacobian()
    ...
    def compute(self, norm: str = "l2"):
        ...
        norm = norm.lower()
        if norm == "l2":
            return self.l2_error_time()
        elif norm in "linf":
            return self.linf_error_time()
        elif norm == "h1_semi":
