...
class OSWRHeat():
    def __init__(self, heat_problem: HeatProblem, decinfo: DecompositionInfo):
        """
        Initialize an Overlapping Schwarz Waveform Relaxation (OSWR) problem 
        ...
        heat_problem : HeatProblem
            ...
            to be solved using the OSWR method.
    ...
    def _construct_subfemspaces(self) -> dict[int, FEMSpace]:
        ...
        space, and degree as the global finite element space (`heat_problem.femspace`).
    ...
    def _construct_subproblems(self, ntime: int, data: dict[int, np.ndarray]) -> dict[int, HeatProblem]:
        """
        Return the Heat problems for each subdomain based on the global 
        Heat problem and the provided data.

        This function constructs a `HeatProblem` instance for each subdomain by using the 
        ...
        account the contributions from all neighbouring subdomains sharing the boundary 
        nodes.
        ...
        Returns
        -------
        dict[int, HeatProblem]
            A dictionary mapping each subdomain ID to its corresponding `HeatProblem` instance.
        """
        subproblems = {}
        ...
        for subdomain_id in self.subids:
            ...
            subproblems[subdomain_id] = HeatProblem(femspace = subfem, t0 = t0, T = T, f = source, g = g_local, h = h_local)
        return subproblems
    ...
    def construct_dirichlet_bc(self, domainID: int, ntime: int, data: dict[int, np.ndarray]) -> np.ndarray:
        ...
        neighbouring subdomains, the boundary value is computed as the average of 
        contributions from all neighbouring subdomains sharing that node. However, 
        if any of the shared entries corresponds to subdomain 0 (the original global domain), 
        the corresponding Dirichlet value is used directly.
    ...
    def solve(self, time_grid: np.ndarray, theta: float = 1.0, lift: Literal['nodal', 'harmonic'] = 'nodal', 
              solver: LinearSolver = DirectSolver(), criterion: Callable = max_difference, maxiter: int = 100, 
              tol: float = 1e-9, omega: float = 1.0, combine: bool = True, histconfig: Optional[HistoryConfig] = None, 
              store_solution: Optional[list | bool] = None) -> dict[int, np.ndarray] | np.ndarray:
        """
        ...
        combine : bool, default = True
            If True, the final global solution is assembled from the subdomain solutions. 
            If False, a dictionary of subdomain solutions is returned.
        store_solution : list | bool, optional
            If True, the subdomain solutions for all iterations are stored. If a list of iteration indices is provided, only the subdomain 
            solutions for those iterations are stored. The stored solutions are returned in a dictionary with keys as iteration indices and 
            values as dictionaries of subdomain solutions, where each subdomain solution is an array of shape `(nnodes, ntime)` representing 
            the local solution for that subdomain at all time steps.
        ...
        solution : dict[int, np.ndarray] | np.ndarray
            If `combine` is True, returns a global solution array of shape `(nnodes, ntime)`. 
            If `combine` is False, returns a dictionary mapping subdomain IDs to their local 
            solution arrays of shape `(nnodes, ntime)`, where nnodes is the number of nodes 
            in the subdomain.
        """
        ...
        logger.info(f"dim: {self.femspace.dim}D | " f"subdomains: {self.nsub} | " f"version: {self.version} | "
                    f"theta: {theta} | " f"lift: {lift} | " f"maxiter: {maxiter} | " f"tol: {tol:.2e} | " f"omega: {omega}")
        logger.info("-"*80)
        ...
        subproblems = self._construct_subproblems(ntime = ntime, data = data)
    
        # Overlapping Schwarz Waveform Relaxation iterations
        error: float = float("inf")
        for iter in range(maxiter):
            new_data = {}
            # Solve local Heat problems 
            for subdomain_id in self.subids:
