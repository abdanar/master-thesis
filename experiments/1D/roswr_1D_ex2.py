...
def proj_matrices(r: int, ntime: int, femspace: FEMSpace, roswrproblem: ROSWRProblem, lift, theta: float, solver) -> dict[int, np.ndarray]:
    ...
    snapshot_matrix[femspace.interior_nodes,:] = roswrproblem.heat_problem.solve(time_grid = time_grid, lift = lift, theta = theta, solver = solver, 
                                                g_new = np.vstack([np.zeros(ntime), np.exp(-time_grid)]), homogeneous = True)
...
global_fig, global_ax = plot_wireframe(vert1D, time_grid, heat_solution1D.T, xlabel=r'$x$', ylabel=r'$t$', title = r'Solution of the 1D Heat Equation', xlim=(0, 1), ylim=(t0, T))
...
# Save the figures
plt.tight_layout()
subdomain_fig.savefig(f"figures/1D/roswr/ex2/subdomains_{T}.png", dpi = 300, bbox_inches = 'tight')
iterates_fig.savefig(f"figures/1D/roswr/ex2/iterates_{T}.png", dpi = 300, bbox_inches = 'tight')
plt.show()
