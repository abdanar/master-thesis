...
To apply boundary conditions, we define the \textit{reconstructed Dirichlet vector} 
\begin{equation*}
    \hat{d}^{l+1, k+1}_{i} \in \mathbb{R}^{n_{i}},
\end{equation*}
defined as
\begin{equation*}
...
corresponding to the physical boundary nodes. 

Then, the system we obtained above can be written in the block-matrix representation as 
\begin{equation*}
    \begin{bmatrix}
        \Phi_{i, I}^{T} & \Phi_{i, D}^{T}
    \end{bmatrix}
    \begin{bmatrix}
        K_{i, II} & 0\\
        0 & I_{\lvert B_{i}\rvert}
    ...
    \end{bmatrix}
    \begin{bmatrix}
        L_{i, I}\left(M_{i}\Phi_{i}\beta_{i}^{l, k+1} + \Delta t F_{i}^{l+1}\right) - K_{i, ID}\hat{d}^{l+1, k+1}_{i, D}\\
        \hat{d}^{l+1, k+1}_{i, D}
    \end{bmatrix},
\end{equation*}
where $b_{i, I} = L_{i, I}b_{i}$ and $\hat{d}^{l+1, k+1}_{i, D} = L_{i, D}\hat{d}^{l+1, k+1}_{i}$. Then, we can write this as
\begin{equation*}
    \left(\Phi_{i, I}^{T}K_{i, II}\Phi_{i, I} + \Phi_{i, D}^{T}\Phi_{i, D}\right)\beta_{i}^{l+1, k+1} = \Phi_{i, I}^{T}L_{i, I}\left(M_{i}\Phi_{i}\beta_{i}^{l, k+1} + 
    \Delta t F_{i}^{l+1}\right) - \Phi_{i, I}^{T}K_{i, ID}\hat{d}^{l+1, k+1}_{i, D} + \Phi_{i, D}^{T}\hat{d}^{l+1, k+1}_{i, D}
\end{equation*}
which can be reduced to
\begin{equation*}
    \Phi_{i, I}^{T}K_{i, II}\Phi_{i, I} \beta_{i}^{l+1, k+1} = \Phi_{i, I}^{T}L_{i, I}\left(M_{i}\Phi_{i}\beta_{i}^{l, k+1} + 
    \Delta t F_{i}^{l+1}\right) - \Phi_{i, I}^{T}K_{i, ID}\hat{d}^{l+1, k+1}_{i, D} + \Phi_{i, D}^{T}\hat{d}^{l+1, k+1}_{i, D} - \Phi_{i, D}^{T}\Phi_{i, D}\beta_{i}^{l+1, k+1}
\end{equation*}
Since $\hat{d}^{l+1, k+1}_{i} = \Phi_{i}\beta_{i}^{l+1, k+1}$, we get
\begin{equation*}
    \Phi_{i, I}^{T}K_{i, II}\Phi_{i, I} \beta_{i}^{l+1, k+1} = \Phi_{i, I}^{T}L_{i, I}\left(M_{i}\Phi_{i}\beta_{i}^{l, k+1} + 
    \Delta t F_{i}^{l+1}\right) - \Phi_{i, I}^{T}K_{i, ID}\hat{d}^{l+1, k+1}_{i, D}.
\end{equation*}
The interior system is given by
\begin{equation*}
    K_{i, II}\alpha_{i, I}^{l+1, k+1} = b_{i, I} - K_{i, ID}d^{l+1, k+1}_{i, D}.
\end{equation*}
\newpage
We know that for FOM with the Dirichlet boundary condition $\hat{d}^{l+1, k+1}_{i, D}$, the following is satisfied
\begin{equation*}
    \boxed{K_{i, II}\alpha_{i, I}^{l+1, k+1} = L_{i, I}M_{i}\alpha_{i}^{l, k+1} + 
    L_{i, I}\Delta t F_{i}^{l+1} - K_{i, ID}\hat{d}^{l+1, k+1}_{i, D}}.
\end{equation*}
If we assume that
\begin{equation*}
    \alpha_{i}^{l+1, k+1} = 
    ...
    \begin{bmatrix}
        ...
        0
    \end{bmatrix}
    \tilde{\beta}_{i}^{l+1, k+1} + 
    \begin{bmatrix}
        0\\
        \hat{d}_{i, D}^{l+1, k+1}
    \end{bmatrix}
    =
    \Phi_{i}\tilde{\beta}_{i}^{l+1, k+1} + \hat{d}_{i}^{l+1, k+1},
\end{equation*}
where
\begin{equation*}
    \Phi_{i} = 
    \begin{bmatrix}
        \Phi_{i, I}\\
        0
    \end{bmatrix} \in \mathbb{R}^{n_{i}\times r_{i}}, \qquad  \Phi_{i, I} = L_{i, I}\Phi_{i}\in \mathbb{R}^{(n_{i} - \lvert B_{i}\rvert) \times r_{i}}.
\end{equation*}
Once we substitute to the formula, we get
\begin{align*}
    K_{i, II}\Phi_{i, I} \tilde{\beta}_{i}^{l+1, k+1} &= L_{i, I}M_{i}\Phi_{i}\tilde{\beta}_{i}^{l, k+1} + 
    L_{i, I}\Delta t F_{i}^{l+1} + L_{i, I}M_{i}\hat{d}_{i}^{l, k+1} - K_{i, ID}\hat{d}^{l+1, k+1}_{i, D} = \\
    &= L_{i, I}M_{i}\Phi_{i}\tilde{\beta}_{i}^{l, k+1} + 
    \Delta t L_{i, I} F_{i}^{l+1} - L_{i, I}M_{i}\left(\hat{d}_{i}^{l+1, k+1} - \hat{d}_{i}^{l, k+1}\right) - \Delta t L_{i, I}A_{i}\hat{d}^{l+1, k+1}_{i},
\end{align*}
since $\hat{d}^{l+1, k+1}_{i} = L_{i, D}^{T}\hat{d}^{l+1, k+1}_{i, D}$.
Multiplying by $\Phi_{i, I}^{T}$ from left side, we obtain
\begin{equation*}
    \Phi_{i}^{T}K_{i}\Phi_{i} \tilde{\beta}_{i}^{l+1, k+1} = \Phi_{i}^{T}M_{i}\Phi_{i}\tilde{\beta}_{i}^{l, k+1} + 
    \Delta t\Phi_{i}^{T} F_{i}^{l+1} - \Phi_{i}^{T}K_{i}\hat{d}^{l+1, k+1}_{i} + \Phi_{i}^{T}M_{i} \hat{d}_{i}^{l, k+1},
\end{equation*}
since $\Phi_{i}^{T} = \Phi_{i, I}^{T}L_{i, I}$. Thus, we get
\begin{equation*}
    \boxed{K_{r, i} \tilde{\beta}_{i}^{l+1, k+1} = M_{r, i}\tilde{\beta}_{i}^{l, k+1} + 
    \Delta t\Phi_{i}^{T} F_{i}^{l+1} - \Phi_{i}^{T}K_{i}\hat{d}^{l+1, k+1}_{i} + \Phi_{i}^{T}M_{i} \hat{d}_{i}^{l, k+1}}
\end{equation*}
Once we get the reduced solution $\tilde{\beta}_{i}^{l+1, k+1}$, we can reconstruct the full-order solution as
\begin{equation*}
    \boxed{\alpha_{r, i}^{l+1, k+1} = \Phi_{i}\tilde{\beta}_{i}^{l+1, k+1} + \hat{d}_{i}^{l+1, k+1}}.
\end{equation*}
\textbf{This reconstruction is only needed to construct the Dirichlet data for the neighboring subdomains.}

\noindent The initial condition for iteration is 
\begin{equation*}
    \tilde{\beta}_{i}^{0, k + 1} = \Phi_{i}^{T}\left(\alpha_{i}^{0, k + 1} - \hat{d}_{i}^{0, k + 1}\right).
\end{equation*}
Note that if we take
\begin{equation*}
    \Phi_{i, I} = I_{(n_{i} - |B_{i}|)\times (n_{i} - |B_{i}|)},
\end{equation*}
then the Reduced Waveform Relaxation iteration reduces to Overlapping Waveform Relaxation iteration. Then we can combine solutions from
all subdomains as (global reconstruction)
\begin{equation*}
    \boxed{\alpha_{r}^{l+1, k+1} = \sum_{i=1}^{s}R_{i}^{T}U_{i}\Phi_{i}\tilde{\beta}_{i}^{l+1, k+1} \in \mathbb{R}^{n}}
\end{equation*}
\end{document}
